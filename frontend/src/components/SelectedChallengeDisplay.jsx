import "../styles/SelectedChallengeDisplay.css";
...
const SelectedChallengeDisplay = ({
  ...
  useEffect(() => {
    ...
    const fetchUserDetailsByChallengeId = async () => {
      try {
        const [userDetailsData, error] = await getChallengeCreator(
          challenge.id
        );
    ...
    <div className="selected-challenge-modal">
      <div className="selected-challenge-content-box">
        <button
          ...
          className="selected-challenge-close-button"
        >
          X
        </button>
        ...
        <p>
          {challenge.description ||
            "Detailed description of the selected challenge."}
        </p>
        ...
        <div className="related-posts-section">
          ...
          <div className="post-scroll-container">
            {relatedUserPosts.length > 0 ? (
              relatedUserPosts.map((userpost) =>
                userpost.posts.map((post) => (
                  <div key={post.postId} className="post-item">
                    <p>
                      <strong>User: {userpost.username}</strong>
                    </p>
